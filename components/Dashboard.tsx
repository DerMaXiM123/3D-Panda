...
const Dashboard: React.FC<DashboardProps> = ({ onViewChange, user }) => {
  ...
  useEffect(() => {
    Promise.all([db.getFilaments(), db.getFriends(), db.getPrinters(), db.getPrintLogs()]).then(([f, p, pr, l]) => {
      setFilaments(f); setFriends(pr); setPrinters(p); setLogs(l);
    });
  }, []);
      ...
      <header className="relative">
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8 mb-12">
          <div className="space-y-4">
            ...
            <p className="text-slate-500 font-bold uppercase text-[11px] tracking-[0.5em] italic opacity-60 ml-2">Hardware ID: {user.username.toUpperCase()}-09-PRO</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
             ...
             >
                ...
                <div>
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Job Scheduler</p>
                   <p className="text-lg font-black italic text-white leading-none mt-1">{logs.length % 3 + 1} Queue <span className="text-[10px] text-orange-500 uppercase ml-2 tracking-tighter">Pending</span></p>
                </div>
             </button>
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 glass rounded-[56px] p-12 border-white/5 bg-slate-900/60 relative overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
             <TelemetryGauge label="Node Latency" value="0.4ms" progress={98} color="blue" />
             <TelemetryGauge label="Neural Link" value="Stable" progress={92} color="purple" />
             <TelemetryGauge label="Sync Mesh" value="Active" progress={100} color="emerald" />
          </div>
        </div>
      ...
      <section className="space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
           <div className="space-y-6">
              <h3 className="text-[10px] font-black text-blue-500 uppercase tracking-widest px-4">CAD Generatoren</h3>
              <div className="grid grid-cols-1 gap-3">
                 <MatrixItem icon={<Box size={18}/>} label="Brick Studio" view={AppView.CREATOR_BRICK} onClick={onViewChange} color="red" />
                 <MatrixItem icon={<Maximize2 size={18}/>} label="Calibration Cube" view={AppView.CREATOR_CUBE} onClick={onViewChange} color="blue" />
           ...
           <div className="space-y-6">
              <h3 className="text-[10px] font-black text-purple-500 uppercase tracking-widest px-4">Neural Logic</h3>
              <div className="grid grid-cols-1 gap-3">
                 <MatrixItem icon={<ScanFace size={18}/>} label="Surface Inspector" view={AppView.VISION_LAB} onClick={onViewChange} color="blue" />
                 <MatrixItem icon={<MessageSquare size={18}/>} label="Expert Engine" view={AppView.CHAT} onClick={onViewChange} color="purple" />
                 <MatrixItem icon={<FileCode size={18}/>} label="G-Code Analyst" view={AppView.GCODE_ANALYST} onClick={onViewChange} color="orange" />
              </div>
           </div>
           <div className="space-y-6">
              <h3 className="text-[10px] font-black text-orange-500 uppercase tracking-widest px-4">Logistik & Mesh</h3>
              <div className="grid grid-cols-1 gap-3">
                 <MatrixItem icon={<ListTodo size={18}/>} label="Job Scheduler" view={AppView.PRINT_QUEUE} onClick={onViewChange} color="orange" />
                 <MatrixItem icon={<Users size={18}/>} label="Maker Network" view={AppView.FRIENDS} onClick={onViewChange} color="emerald" />
                 <MatrixItem icon={<Package size={18}/>} label="Material Registry" view={AppView.INVENTORY} onClick={onViewChange} color="orange" />
              </div>
           </div>
        </div>
      </section>
